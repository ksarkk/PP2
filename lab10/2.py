...
class DBManager:
    ...
    def getRecords(self):
        query = """SELECT * FROM snake_table ORDER BY points DESC"""
        self.cursor.execute(query)
        return self.cursor.fetchall()

    def insertIfNotExists(self, username):
        query = f"""INSERT INTO snake_table(name, points, level)
                SELECT '{username}', 0, 0
                WHERE NOT EXISTS (SELECT 1 FROM snake_table WHERE name = '{username}')
                """
        self.cursor.execute(query)
        self.commit()

    def updateIfBest(self, username, points, level):
        query = f"""UPDATE snake_table
...
class Fruit(GameObject):
    ...
    def __init__(self):
        ...
        self.image = pygame.transform.scale(pygame.image.load(os.path.abspath(
            f"lab10/{Constants.fruits[self.weight]}.png")).convert_alpha(),(cell_size, cell_size))
...
class RecordsMenu:
    def drawMenu(self, where, data):
        ...
        recordsMenu = pygame_menu.Menu(title='RECORDS', width=450, height=300, enabled=False, theme=theme, onclose=lambda: recordsMenu.disable())
        recordsMenu.enable()
...
class Game:
    ...
    def drawPlayBtn(self):
        restartImg = pygame.image.load(os.path.abspath("lab10/restart.png")).convert_alpha()
    ...
    def drawMenuBtn(self):
        menuImg = pygame.image.load(os.path.abspath("lab10/menu.png")).convert_alpha()
