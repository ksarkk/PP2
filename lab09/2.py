...
class Orange() :
    ...
    def draw_orange(self) :
        orange_rect = pygame.Rect(self.pos.x * CELL_SIZE, self.pos.y * CELL_SIZE, CELL_SIZE, CELL_SIZE)
...
class Main() :
    ...
    def draw_elements(self) :
        ...
        self.orange.draw_orange()
        self.wall.draw_wall()

    def collision_food(self) :
        global SCORE
        if self.orange.pos == self.snake.body[0] :
            self.snake.body.insert(len(self.snake.body), self.snake.body[1])
            self.snake.body.insert(len(self.snake.body), self.snake.body[1])
            self.orange.random()
            pygame.time.set_timer(orange_timer, 5000)
            SCORE += 2
...
orange = pygame.image.load('lab08\Orange_.png').convert_alpha()
...
orange_timer = pygame.USEREVENT + 1
pygame.time.set_timer(orange_timer, 5000)
